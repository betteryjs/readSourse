...
int task_insert(struct rb_root *root, struct Task *task)
{
    ...
    while (*tmp) {
        struct Task *thisTask = rb_entry(*tmp, struct Task, rb_node);

        parent = *tmp;
        if (task->val < thisTask->val)
            tmp = &((*tmp)->rb_left);
        else if (task->val > thisTask->val)
            tmp = &((*tmp)->rb_right);
        else
            return -1;
    }
...
int main()
{
    ...
    for (node = rb_last(&task_tree); node; node = rb_prev(node))
        printf("val = %d\n", rb_entry(node, struct Task, rb_node)->val);

    // delete
    struct Task *data = task_search(&task_tree, 5);
    if (data) {
